...
func _build_vitality_section() -> void:
	...
	for i in range(_vitality_rewards.size()):
		var reward_level = _vitality_rewards[i]
...
func _on_vitality_reward_clicked(index: int) -> void:
	if index >= _vitality_rewards.size():
		return

	var reward_level = _vitality_rewards[index]
